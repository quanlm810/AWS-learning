...
- Application load balancer is a layer 7 load balancer (HTTP)
- Load balancing to multiple HTTP applications across machines (target groups)
- Support for HTTP and WebSocket
- Support routing table to different groups: path based, hostname based, queries/params based
- ALB are great for micro-services and container based applications
- Has port mapping feature
#### Target groups
- EC2 instances (can be managed by ASG) - HTTP
- ECS tasks (managed by ECS itself) - HTTP
- Lambda functions - HTTP requests are translated to JSON events
- IP Addresses - must be private IP address
**Note:** Healthcheck will be done at target groups level
**Note:**
- The application don't see client IP directly:
    - Client IP address is in X-Forwarded-For header
    - We can also get port (X-Forwarded-Port) and proto (X-Forwarded-Proto)
### Network Load Balancer
- Network load balancer is a layer 4 load balancer (TCP, UDP, TLS,...)
- Very high performance, can handle millions of request per seconds, ultra low latency
- NLB only have 1 static IP per AZ and support assigning Elastic IP
#### Target groups
- EC2 instances
- IP Addresses - must be private IP address
- Application Load Balancer
**Note:** Health check support TCP, HTTP, HTTPS protocols
### Gateway Load Balancer
- Deploy, scale and manage a fleet of 3rd party network virtual appliances in AWS
- Operate at layer 3 (Network Layer) - IP Packets
- A combination of following functions:
...
- It is possible to always route user to the same instance behind the load balancer
- This works for Classic Load Balancer, Application Load Balancer and Network Load Balancer
- The cookie is used for stickiness and have an expiration date (Note that NLB don't need cookie to work)
- Make sure the user doesn't lose session data
- Enabling stickiness may cause imbalance in the load because some user are more sticky than other
#### Application-based cookies
- Custom cookie generated by the target
    - Include any additional attributes required by the application
    - Cookie name must be specified individually for each target group
    - Reserved name: AWSALB AWSALBAPP, or AWSALBTG (for used in ELB)
- Application cookie
    - Generated by the cookie itself
    - Cookie name will be AWSALBAPP
#### Duration-based cookies
- Cookie generated by the load balancer
- Cookie name is AWSALB for ALB, AWSELB for CLB
### Cross-Zone load balancing
- With cross-zone load balancing the load is distributed evenly across all instances in all AZ 
